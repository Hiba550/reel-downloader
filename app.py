...
def share_target():
    """Handle shared URLs from other apps (PWA Share Target)"""
    shared_url = request.form.get('url', '')
    shared_text = request.form.get('text', '')
    shared_title = request.form.get('title', '')
    
    # Extract Instagram URL from shared content
    instagram_url = shared_url or shared_text or shared_title
    
    # Store the URL in session and redirect with background flag
    if instagram_url and 'instagram.com' in instagram_url:
        session['shared_url'] = instagram_url
        session['background_mode'] = True  # Flag for minimal UI
        return redirect(url_for('index'))
    else:
        return redirect(url_for('index', error='No valid Instagram link was shared'))
...
def index():
    ...
    # Get shared URL from session (if coming from share-target)
    shared_url = session.pop('shared_url', None)
    background_mode = session.pop('background_mode', False)
    ...
    return render_template('index.html', error=error_message, shared_url=shared_url, background_mode=background_mode)

@app.route('/test-overlay')
def test_overlay():
    """Test page for overlay functionality"""
    return render_template('test_overlay.html')
...
# For production (Azure), Gunicorn will use the 'app' object directly